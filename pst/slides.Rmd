...
## Introduction
- Machine Learning: a set of **methods** that **computers** use to make and improve **predictions** or behaviors based on **data**.
    - Machine Learning Model: the learned program that maps inputs to predictions.
![ml](../img/ml.png)
        - Black Box Model: a system that does not reveal its internal mechanisms (vs. white box model/interpretable model)

## Introduction (cont.)
- Interpretable/Explainable Machine Learning: methods and models that make the behavior and predictions of machine learning systems *understandable* to humans.
![big-picture](../img/big-picture.png)
- This talk focus on introducing explainable ML methods and tools, or *how* to interpret ML results.

## Taxonomy of explanaible ML methods
- intrinsic vs post-hoc: interpretability is achieved 
    - intrinsic: by restricting the complexity of the machine learning model that are considered interpretable due to their simple structure, such as short decision trees or sparse linear models. Inherently nterpretable models are models with (learned) structures and learned parameters which can be assigned a certain interpretation. 
...
## Taxonomy of explanaible ML methods (cont.)
...
- analyze components of interpretable models: component analysis is always model-specific, because it is tied to the structure of the model
- analyze components of model complex models: feature map of CNN, minimal depth distribution and the Gini importance of random forest could be used to qualify the feature importance

summary of drawbacks of mdoel-specific interpretable method: it is tied to specific model, and it does not combine well with the common model selection approach in ML, where on usually searches over a large class of different ML models via cross-validation


- study sensitivity of an ML model, model-agnostic and work by manupulating input data and analyzing the respective model prediction. 
    - explain individual predictions: Shaply value; counterfactual explanations: explain predictions in the form of what-if scenarios.
    - example global behavior [@zhang2017]
...
- Shapley Value: collaborative game theory, provides an answer on how to fairly share a payout among the players of a collaborative game. The collaborative game idea can be applied to ML where features (i.e., the players) collaborative to make a prediction (i.e. the payout)
